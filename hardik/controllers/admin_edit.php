<?php
    ...
    if (isset($_POST['update']))
    {
		...
		$update_query = execute_query($update_sql, $param);
        ...
        {   
            ...
            {                
                ...
                /*$sql = "SELECT filepath FROM tbl_files where filepath=? ";
				...
				$img_sql = fetch_row($sql, $param);

                //chceking whether image already exists or not
				$result = count($img_sql);
...
**********************************************************************************************************************/
    ...
    if (isset($_GET['action']) && isset($_GET['edit_id']))
    {
        if ($_GET['action'] != 'edit')
        {
            header('location:users.php?er=no such action found');            
        }
        else
        {
